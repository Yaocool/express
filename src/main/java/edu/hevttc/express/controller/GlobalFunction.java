...
import edu.hevttc.express.enums.*;
...
public class GlobalFunction {
    ...
    public EntityWrapper<Express> getExpressWrapper(ExpressSelectWrapper esw) {
        ...
        if (StringUtils.isNotBlank(esw.getUserId())) {
            entityWrapper.eq("userId", esw.getUserId());
        }
    ...
    public EntityWrapper<SysUser> getSysUserWrapper(SysUserSelectWrapper usw) {
        ...
        if (StringUtils.isNotBlank(usw.getEmail())) {
            entityWrapper.eq("email", usw.getEmail());
        }
        return entityWrapper;
    }
    ...
    public String iso8859ToUtf8(String x) throws UnsupportedEncodingException {
        if (StringUtils.isNotBlank(x)) {
            // 判断是否是iso8859-1编码格式
            if (x.equals(new String(x.getBytes(), "iso8859-1"))) {
